...
router.get("/", validateToken, async (req, res) => {
  const todo = await Todo.find({ "who.id": req.user.id });
...
router.delete("/:id", validateToken, async (req, res) => {
  ...
  const todo = await Todo.findOneAndDelete({ _id: id, "who.id": req.user.id }); // delete todo
...
router.put("/:id", validateToken, async (req, res) => {
  ...
  res.status(201).json({ todoData }); // status 201 successfully Update
});
...
router.post("/", validateToken, async (req, res) => {
  ...
  const todoData = todoDTO(todo);
  res.status(201).json({ todoData }); // status 201 successfully post
});
